...
const server = app.listen(1337, () => {
    ...
    if (
        ...
        process.env.DOMAIN_NAME === null ||
        process.env.DOMAIN_NAME.length === 0
    ) {
        console.log(
            chalk.redBright(
                `process.env.DOMAIN_NAME is undefined... this script requires at least one valid FQDN to which the public ip of this network will be mapped`
            )
        );
        server.close();
        return;
